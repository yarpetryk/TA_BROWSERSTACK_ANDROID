...
class ConfigImporter:
    ...
    def config_path_to_apk(self):
        # Validate and return the Path Firefox APK Name from the config data
        config = self.config()
        if 'path_to_apk' not in config:
            raise Exception('The config file does not contain "path_to_firefox_apk"')
        return config['path_to_apk']
